...
bool CSurface::OnDraw(SDL_Surface* surfDest, SDL_Surface* surfSrc, int x, int y, int x2, int y2, int w, int h)
{
    if(surfDest==NULL || surfSrc==NULL)
        return false;
...
bool CSurface::Transparent(SDL_Surface* Surf_Dest, int R, int G, int B, bool alpha) {
    if(Surf_Dest == NULL) {
        return false;
    }
